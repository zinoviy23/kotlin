...
class FirSealedClassInheritorsTransformer : FirTransformer<Nothing?>() {
    ...
    override fun transformFile(file: FirFile, data: Nothing?): CompositeTransformResult<FirDeclaration> {
        val sealedClassInheritorsMap = mutableMapOf<FirSealedClass, MutableList<FirClassSymbol>>()
    ...
    private class InheritorsTransformer(private val inheritorsMap: MutableMap<FirSealedClass, MutableList<FirClassSymbol>>) : FirTransformer<Nothing?>() {
        override fun <E : FirElement> transformElement(element: E, data: Nothing?): CompositeTransformResult<E> {
    ...
    private object InheritorsCollector : FirDefaultVisitor<Unit, MutableMap<FirSealedClass, MutableList<FirClassSymbol>>>() {
        override fun visitElement(element: FirElement, data: MutableMap<FirSealedClass, MutableList<FirClassSymbol>>) {}

        override fun visitFile(file: FirFile, data: MutableMap<FirSealedClass, MutableList<FirClassSymbol>>) {
            file.declarations.forEach { it.accept(this, data) }
        }

        override fun visitRegularClass(regularClass: FirRegularClass, data: MutableMap<FirSealedClass, MutableList<FirClassSymbol>>) {
            ...
            for (typeRef in regularClass.superTypeRefs) {
                val parent = extractClassFromTypeRef(symbolProvider, typeRef) as? FirSealedClass ?: continue
                val inheritors = data.computeIfAbsent(parent) { mutableListOf() }
                inheritors += regularClass.symbol
            }
        }
