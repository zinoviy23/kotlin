...
open class KaptWithoutKotlincTask @Inject constructor(private val workerExecutor: WorkerExecutor) : KaptTask() {
    ...
    private inline fun <reified T : SubpluginOption> List<SubpluginOption>.findOption(name: String): T? {
        for (option in this) {
            if (option.key == name && option is T) {
                return option
            }
        }
    ...
    private fun CompositeSubpluginOption.toMap(): Map<String, String> {
        val result = LinkedHashMap<String, String>(originalOptions.size)
        for (option in originalOptions) {
            result[option.key] = option.value
        }
        return result
    }
